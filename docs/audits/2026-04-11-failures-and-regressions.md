...
| FB-004 | Assessment model migration | Move deterministic scoring from skill-family level to skill-objective (TGO/ELO) level and remove Draft Signals UI while preserving historical review data. | Current family-level scoring obscures objective-level measurement and progression clarity; historical data continuity must be protected during migration. | Open | Execute `docs/objective-level-assessment-migration-plan.md` with phased rollout, mixed-era compatibility tests, and objective-coverage CI gates. |
