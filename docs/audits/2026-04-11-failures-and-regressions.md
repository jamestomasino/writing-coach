...
| FB-003 | Skill taxonomy governance | 15 skill families are defined in the canonical catalog but not currently mapped by any public TGO tree. | Unused families can confuse product language and make coverage metrics look inconsistent if shown as active curriculum. | Open | Review whether to: (a) map into new TGO trees, (b) keep as reserved/internal-only, or (c) prune from canonical catalog. Add explicit “active vs reserved” taxonomy policy and UI rules. |
| FB-004 | Assessment model migration | Move deterministic scoring from skill-family level to skill-objective (TGO/ELO) level and remove Draft Signals UI while preserving historical review data. | Current family-level scoring obscures objective-level measurement and progression clarity; historical data continuity must be protected during migration. | Resolved | Completed phased rollout from `docs/objective-level-assessment-migration-plan.md`: objective-level scoring is primary for new reviews, mixed-era read/compare compatibility is in place, Draft Signals learner UI is removed, objective coverage gates are enforced, and calibration snapshots now prefer objective-era deterministic/hybrid rows with legacy fallback for historical submissions. |
